...
// ■ MrTS_EventNames
//------------------------------------------------------------------------------
// 　事件名称           注释<Name:大友家 宗像城, 5>
//==============================================================================
/*:
 * @plugindesc | 本地 | 允许事件来显示他们上面的名字。
 ...
 */
(function(){
	var parameters=PluginManager.parameters('事件头顶名称');//配对插件名称
	var paramDefaultRange=Number(parameters['默认范围']||5);
	var paramFontSize=Number(parameters['字体大小']||24);
	var paramFade=Boolean(parameters['Fade']&&parameters['Fade'].toLowerCase()!=="false"||true);//淡出
	var paramFadeTimer=Number(parameters['Fade Timer']||30);//淡出计时
	...
	Game_Event.prototype.initialize=function(mapId,eventId){
		...
		var meta=this.event().meta;this._displayOverheadType=null;
		var nameMeta=null;
		if(meta.Icon){
		...
		if(this.list()){
			for(action of this.list()){
				if(action.code=="108"){
					...
					if(!match)match=regexPictureName.exec(a);if(!match)match=regexIconName.exec(a);if(match){
						overheadType=match[1].toLowerCase();overheadName=match[2];
	...
	Game_Event.prototype.update=function(){
		...
		if(this._pageIndex!=this._overheadPageIndex)this.updateOverheadData();
	};
	var _SpriteCharacter_initialize=Sprite_Character.prototype.initialize;
	//
	Sprite_Character.prototype.initialize=function(character){
		_SpriteCharacter_initialize.call(this,character);this._npAdded=false;
	};
	//
		...
		if(this._spriteOverhead){
			...
			if(this._character._displayOverheadRange<Math.abs(($gamePlayer.x- this._character.x))+ Math.abs(($gamePlayer.y- this._character.y))){
				if(paramFade){
					...
					this._spriteOverhead.visible=false;
			}else{
				if(paramFade){
	...
	Sprite_Character.prototype.updateOverheadData=function(){
		if(!this._npAdded){
			this._npAdded=true;this._spriteOverhead=new Sprite();
			this._spriteOverhead.z=7;
			this.updateOverhead();
			this.parent.addChild(this._spriteOverhead);
		}
		if((this._character._displayOverheadType&&this._character._displayOverheadType!=this._currentOverheadType)||(this._character._displayOverheadName&&this._character._displayOverheadName!=this._currentOverheadName)||(this._character._displayOverheadRange&&this._character._displayOverheadRange!=this._currentOverheadRange))
			this.updateOverhead();
	};
})();